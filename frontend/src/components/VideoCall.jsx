...
const VideoCall = () => {
    ...
    // Effect to initialize the call and set up WebRTC
    useEffect(() => {
        const servers = {
            iceServers: [
    ...
    // Effect for handling all socket signaling
    useEffect(() => {
        ...
        const handleCallEnded = () => {
             ...
             endCall(false); // Don't emit another end-call event
        };
    ...
    const endCall = (emitEvent = true) => {
        ...
        // Redirect based on whether user is a doctor or patient
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-900 text-white">
            ...
            <div className="bg-gray-800 p-4 flex justify-center items-center gap-4">
                <button onClick={toggleMute} className={`p-3 rounded-full transition-colors ${isMuted ? 'bg-red-500 text-white' : 'bg-gray-600 hover:bg-gray-500'}`}>
                    {isMuted ? <MicOff size={24} /> : <Mic size={24} />}
                </button>
                <button onClick={toggleCamera} className={`p-3 rounded-full transition-colors ${isCameraOff ? 'bg-red-500 text-white' : 'bg-gray-600 hover:bg-gray-500'}`}>
                     {isCameraOff ? <VideoOff size={24} /> : <Video size={24} />}
                </button>
                 <button onClick={() => endCall(true)} className="p-3 rounded-full bg-red-600 hover:bg-red-700 transition-colors">
                    <PhoneOff size={24} />
                </button>
            </div>
        </div>
